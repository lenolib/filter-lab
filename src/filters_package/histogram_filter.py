...
class Histogram_filter(Filter_base):
    ...
    def process(self, input_images, connected_outs):
        ...
        hist_vals = self.calcHistogramCV(src, no_of_bins, mini, maxi)
...
#            self.setParamContent("Histogram plot",self.hist_fig, emitSignal=True)
            self.abortQuery()
            self.pixmap = arr2pixmap(plotfig2image(self.hist_fig)) #Takes on the order of 150 ms
        return {'Histogram':hist_vals, 'Bins':numpy.hstack((x_vals,[maxi]))}
    
    def calcHistogramCV(self, img, no_of_bins, mini, maxi):
        if img.dtype != 'f' or img.dtype != 'i1':
            img = img.astype('f')
        imgs = pycv.vector_Mat([ pycv.asMat(img) ])
        channels = pycv.vector_int([0])
        pycvmask = pycv.Mat()
        hist = pycv.MatND()
        histSize = pycv.vector_int([no_of_bins])
        f = pycv.vector_float32([float(mini),float(maxi)])
        ranges = pycv.vector_vector_float32([f])
        pycv.calcHist(imgs,channels,pycvmask,hist,histSize,ranges)
        return hist.ndarray
