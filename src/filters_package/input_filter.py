...
or select the first image in a sequence to be processed.",
    ...
    def __getstate__(self):
        ...
        del pickle_dict['video_capture']
        del pickle_dict['camera_capture']
        return pickle_dict
...
#        return res
    ...
    def deleteCaptures(self):
        if self.camera_capture is not None: 
            self.camera_capture.release()  #temporary fix
            self.camera_capture = None
...
#        print "'%s' frame counter: %d" %(self.name,self.frame_counter)
    ...
    def processCamera(self, connected_outs, output_choice): 
        if self.camera_capture is None: 
            self.camera_capture = pycv.VideoCapture(0)
            if not self.camera_capture.isOpened():
                ...
                self.frame_counter = 0
        frameC = pycv.Mat()
        self.camera_capture >> frameC
        if frameC.empty():
#                print "Could not get frame from camera"
            return FAIL
        frame = frameC.ndarray    
        processed_images = {}    
        if frame.ndim == 3:
    ...
    def processVideo(self, connected_outs, output_choice):
        ...
        if self.video_capture is None:
            if video_path is None:
                return FAIL
            self.video_capture = pycv.VideoCapture(video_path)        
        ...
        frameC = pycv.Mat()
        self.video_capture >> frameC
        if frameC.empty():
            if (self.getParamContent('Loop video or sequence') == 'Yes' and
                self.video_capture.get(pycv.CV_CAP_PROP_POS_AVI_RATIO == 1) ):                
                self.video_capture.set(pycv.CV_CAP_PROP_POS_AVI_RATIO,0)
                self.frame_counter = 0
                self.video_capture >> frameC
    ...
    def processImage(self, connected_outs, output_choice, image_path=None):
        frame = None
        if self.camera_capture is not None: self.camera_capture = None  #temporary fix
        if self.video_capture is not None: self.video_capture = None  #temporary fix
